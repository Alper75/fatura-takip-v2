...
export default function GidenElogoFaturalar() {
  ...
  const fetchFaturalar = async () => {
    ...
    try {
      ...
      console.error('Giden faturalar alınamadı:', error);
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto p-4 sm:p-6 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Giden E-Faturalar (eLogo)</h2>
          <p className="text-muted-foreground mt-1">eLogo üzerinden kestiğiniz e-Faturaları görüntüleyin, filtreleyin ve kaydedin.</p>
        </div>
      ...
      <Card className="border shadow-sm bg-white overflow-hidden">
        <CardHeader className="border-b bg-gray-50/70 pb-4">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <CardTitle className="text-lg font-bold text-gray-800">Giden Fatura Listesi</CardTitle>
              <CardDescription>
                Müşterilerinize düzenlediğiniz e-Faturalar.
              </CardDescription>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="relative min-w-[200px]">
                ...
                <input
                  type="text"
                  placeholder="Alıcı, VKN veya No Ara..."
                  value={searchTerm}
            ...
            <Table>
              <TableHeader className="bg-slate-50 border-b">
                <TableRow>
                  ...
                  <TableHead className="font-semibold text-slate-700">Alıcı / VKN</TableHead>
                  ...
                  filteredFaturalar.map((f, i) => {
                    ...
                    return (
                      <TableRow key={i} className={isSaved ? "bg-emerald-50/40 hover:bg-emerald-50/70" : "hover:bg-slate-50/80"}>
                        ...
                        <TableCell>
                          <div className="font-medium text-slate-900">{f?.senderName || f?.aliciUnvan || 'Bilinmiyor'}</div>
                          <div className="text-xs text-slate-500 font-mono mt-0.5">VKN: {f?.senderVkn || f?.aliciVkn || '-'}</div>
                        </TableCell>
                        <TableCell>
