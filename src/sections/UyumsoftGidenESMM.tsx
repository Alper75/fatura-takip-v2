...
export default function UyumsoftGidenESMM() {
  ...
  const thirtyDaysAgo = new Date(new Date().setDate(new Date().getDate() - 60)).toISOString().split('T')[0];
  ...
  const fetchFaturalar = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/uyumsoft/giden-esmm?baslangic=${startIso}&bitis=${endIso}`, {
      ...
      if (data.success) {
        ...
        toast.error(data.message || 'Makbuzlar alınamadı');
      }
    } catch (error) {
      console.error('Giden makbuzlar alınamadı:', error);
  ...
  const handleImportInvoice = (fatura: any) => {
    setSelectedInvoices([fatura.uuid || fataturaUuid(fatura)]);
    setShowPreviewModal(true);
  };

  const fataturaUuid = (f: any) => f?.uuid || f?.documentUuid || f?.documentId;
  ...
  const unsavedCount = useMemo(() => faturalar.filter(f => !checkIsSaved(f)).length, [faturalar, savedInvoices, satisFaturalari]);
  ...
  const filteredFaturalar = useMemo(() => {
    return faturalar.filter(f => {
      ...
      if (searchTerm) {
        ...
        return matchName || matchVkn || matchNo;
      }
      return true;
    });
  }, [faturalar, statusFilter, searchTerm, savedInvoices, satisFaturalari]);

  const selectableFiltered = useMemo(() => filteredFaturalar.filter(f => !checkIsSaved(f)), [filteredFaturalar, savedInvoices, satisFaturalari]);

  return (
    <div className="space-y-6 max-w-7xl mx-auto p-4 sm:p-6 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Giden e-SMM (Uyumsoft)</h2>
          <p className="text-muted-foreground mt-1">Uyumsoft portalınızdaki e-SMM makbuzlarını görüntüleyin, filtreleyin ve kaydedin.</p>
        </div>
      ...
      <Card className="border shadow-sm bg-white overflow-hidden">
        <CardHeader className="border-b bg-gray-50/70 pb-4">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <CardTitle className="text-lg font-bold text-gray-800">e-SMM Listesi</CardTitle>
              <CardDescription>
                Uyumsoft üzerinden düzenlenen serbest meslek makbuzları.
              </CardDescription>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="relative min-w-[200px]">
                ...
                <input
                  type="text"
                  placeholder="Müşteri, VKN veya Makbuz No Ara..."
                  value={searchTerm}
                ...
                <TableRow>
                  <TableHead className="w-12 text-center">
                    <input 
                      ...
                      checked={selectableFiltered.length > 0 && selectableFiltered.every(f => selectedInvoices.includes(fataturaUuid(f)))}
                      onChange={(e) => {
                        if (e.target.checked) {
                          const toAdd = selectableFiltered.map(f => fataturaUuid(f));
                          setSelectedInvoices(prev => Array.from(new Set([...prev, ...toAdd])));
                        } else {
                          const toRemove = new Set(selectableFiltered.map(f => fataturaUuid(f)));
                          setSelectedInvoices(prev => prev.filter(id => !toRemove.has(id)));
                        }
                      }}
                    />
                  </TableHead>
                  <TableHead className="font-semibold text-slate-700">Müşteri / VKN</TableHead>
                  <TableHead className="font-semibold text-slate-700">Makbuz No</TableHead>
              ...
              <TableBody className="divide-y">
                {loading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={8} className="h-32 text-center text-slate-500">
                      {statusFilter === 'UNSAVED' 
                        ? 'Tüm makbuzlar sisteme kaydedilmiş.' 
                        : statusFilter === 'SAVED' 
                        ? 'Henüz sisteme kaydedilmiş makbuz yok.' 
                        : 'Makbuz bulunamadı.'}
                    </TableCell>
                  </TableRow>
                ) : (
                      ...
                      <TableRow key={i} className={isSaved ? "bg-emerald-50/40 hover:bg-emerald-50/70" : "hover:bg-slate-50/80"}>
                        <TableCell className="text-center">
                          <input 
                            ...
                            checked={selectedInvoices.includes(itemUuid)}
                            disabled={isSaved}
                            onChange={(e) => {
                              if (e.target.checked) {
                                setSelectedInvoices(prev => [...prev, itemUuid]);
                              } else {
                                setSelectedInvoices(prev => prev.filter(id => id !== itemUuid));
                              }
                            }}
                          />
                        </TableCell>
                        ...
                        <TableCell>
                          ...
                          <div className="text-[11px] text-slate-400 font-mono mt-0.5" title={itemUuid || ''}>
                            {itemUuid ? `${itemUuid.split('-')[0]}...` : '-'}
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="text-xs text-slate-600 truncate max-w-[200px]" title={f?.faturaAciklama}>
                            {f?.faturaAciklama || 'e-SMM Makbuzu'}
                          </div>
                        </TableCell>
                        ...
                        <TableCell>
                          <div className="flex items-center justify-center gap-1.5">
                            <Button 
                              ...
                              onClick={() => handleDownloadPdf(itemUuid)}
                              title="e-SMM PDF İndir"
                            >
      ...
      <IntegrationImportPreviewModal 
        ...
        invoices={faturalar.filter(f => selectedInvoices.includes(fataturaUuid(f)))}
