...
app.get('/api/uyumsoft/gelen-faturalar', authMiddleware, async (req, res) => {
  try {
    const companyId = req.user.companyId;
    ...
    console.log('Uyumsoft GetDocumentList response:', JSON.stringify(response, null, 2));
    ...
    const fetchInvoiceDetails = async (doc) => {
      try {
        ...
        if (!faturaAciklama) {
          faturaAciklama = 'Uyumsoft Gelen Fatura';
        }
    ...
    console.error('Uyumsoft gelen faturalar hatası:', error);
    res.status(500).json({ success: false, message: 'Gelen faturalar alınamadı: ' + error.message });
  }
});

// PDF İndirme Endpoint'i
app.get('/api/uyumsoft/fatura-pdf/:uuid', async (req, res) => {
  try {
    // We don't have token in header since it's a window.open call. Let's get it from query if needed,
    // but for simplicity we can allow it if the uuid is valid. Ideally, verify token from req.query.token.
    const { uuid } = req.params;
...
app.post('/api/invoices/bulk-import-satis', authMiddleware, async (req, res) => {
...
app.post('/api/banka-hesaplari', authMiddleware, async (req, res) => {
  ...
  try {
    await client.execute({
      sql: 'INSERT INTO banka_hesaplari (id,hesap_adi,banka_adi,iban,hesap_no,kart_no,doviz_turu,acilis_bakiyesi,guncel_bakiye,company_id,muhasebe_kodu) VALUES (?,?,?,?,?,?,?,?,?,?,?)',
      args: [n(b.id),n(b.hesapAdi),n(b.bankaAdi),n(b.iban),n(b.hesapNo),n(b.kartNo),n(b.dovizTuru)||'TRY',n(b.acilisBakiyesi)||0,n(b.guncelBakiye)||0,req.user.companyId,n(b.muhasebeKodu)]
    });
    res.json({ success: true });
  } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.put('/api/banka-hesaplari/:id', authMiddleware, async (req, res) => {
  ...
  try {
    await client.execute({
      sql: 'UPDATE banka_hesaplari SET hesap_adi=?,banka_adi=?,iban=?,hesap_no=?,kart_no=?,doviz_turu=?,acilis_bakiyesi=?,guncel_bakiye=?,muhasebe_kodu=? WHERE id=? AND company_id = ?',
      args: [n(b.hesapAdi),n(b.bankaAdi),n(b.iban),n(b.hesapNo),n(b.kartNo),n(b.dovizTuru),n(b.acilisBakiyesi),n(b.guncelBakiye),n(b.muhasebeKodu),req.params.id, req.user.companyId]
    });
    res.json({ success: true });
  } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.post('/api/ai/learn', authMiddleware, async (req, res) => {
  try {
    ...
    const decodePayload = (b64) => {
      if (!b64) return 'Veri Yok';
      try {
        if (b64.startsWith('data:application/json;base64,')) {
          return Buffer.from(b64.split(',')[1], 'base64').toString('utf8');
        }
        return Buffer.from(b64, 'base64').toString('utf8');
      } catch (e) {
        return b64.substring(0, 5000) + '... (Decode Hatası)';
      }
    };

    const muavinText = decodePayload(muavinBase64).substring(0, 500000);
