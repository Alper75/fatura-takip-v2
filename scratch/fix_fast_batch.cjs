...
const fastAccuratePopupCode = `document.addEventListener('DOMContentLoaded', async () => {
                ...
                await chrome.scripting.executeScript({
                    ...
                    func: async (rows) => {
                        ...
                        const getFields = () => {
                            ...
                            // Üstteki gizli eskiTarih'i kesinlikle filtreleyerek kods ile 1:1 tam eşliyoruz
                            const tarihs = Array.from(document.querySelectorAll('input[name="EVRAK_TARIH"], input[name*="evrakTarih"], input[name*="TARIH"]'))
                                                .filter(el => el.name !== 'eskiTarih' && el.id !== 'eskiTarih' && el.type !== 'hidden');
                        ...
                        for (let i = 0; i < rows.length; i++) {
                            ...
                            // Eğer mevcut satır sayısı yetersizse yeni satır ekle
                            if (!fields.kods[i] && i > 0) {
                                addNewRow();
                                let attempts = 0;
                                while (!getFields().kods[i] && attempts < 15) {
                            ...
                            // 3. Evrak Tarihi (1:1 satır tarihi)
...
fs.writeFileSync(popupPath, fastAccuratePopupCode, 'utf8');
console.log('popup.js updated with fast and accurate batch filling!');
