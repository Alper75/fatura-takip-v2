...
const definitiveWorkingPopup = `document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 6. Mahsup Fişine Aktar (Bilanço - fisDetayFormArray uyumlu)
    if (mahsupPasteBtn) {
        mahsupPasteBtn.addEventListener('click', async () => {
            ...
            // Kronolojik sıralama garantisi (1. günden 31. güne doğru)
            const parseTs = (d) => {
                if (!d) return 0;
                ...
                await chrome.scripting.executeScript({
                    ...
                    func: async (rows) => {
                        ...
                        const addNewRow = () => {
                            ...
                            const plusBtn = document.querySelector("button[onclick*='satirEkle'], input[value*='Ekle'], [hotkey*='Alt+E'], [hotkey*='alt+e'], .fa-plus, a[onclick*='satirEkle']");
                            if (plusBtn) {
                                plusBtn.click();
                                return;
                            }
                        ...
                        // Üstteki gizli eskiTarih veya ağaç inputlarını engelleyip sadece fiş tablosu satırlarını alıyoruz
                        const isFişInput = (el) => {
                            if (!el) return false;
                            if (el.type === 'hidden' || el.type === 'checkbox' || el.type === 'radio') return false;
                            if (el.name === 'eskiTarih' || el.id === 'eskiTarih') return false;
                            return true;
                        };

                        const getFields = () => {
                            const kods = Array.from(document.querySelectorAll('input[name*="HESAP_KODU"], input[name*="hesapKodu"], input[id*="hesapKodu"]')).filter(isFişInput);
                            const evrakNos = Array.from(document.querySelectorAll('input[name*="EVRAK_NO"], input[name*="evrakNo"], input[id*="evrakNo"]')).filter(isFişInput);
                            const tarihs = Array.from(document.querySelectorAll('input[name*="EVRAK_TARIH"], input[name*="evrakTarih"], input[name*="TARIH"], input[id*="tarih"]')).filter(isFişInput);
                            const belgeTurs = Array.from(document.querySelectorAll('input[name*="BELGE_TURU"], input[name*="belgeTur"], input[id*="belgeTur"]')).filter(isFişInput);
                            const aciklamas = Array.from(document.querySelectorAll('input[name*="ACIKLAMA"], input[name*="aciklama"], input[id*="aciklama"]')).filter(isFişInput);
                            const borcs = Array.from(document.querySelectorAll('input[name*="BORC"], input[name*="borc"], input[id*="borc"]')).filter(isFişInput);
                            const alacaks = Array.from(document.querySelectorAll('input[name*="ALACAK"], input[name*="alacak"], input[id*="alacak"]')).filter(isFişInput);
                        ...
                        for (let i = 0; i < rows.length; i++) {
                            ...
                            // Eğer i. satır kutusu henüz yoksa yeni satır aç
                            ...
                            // 3. Evrak Tarihi (1:1 satıra özel)
...
fs.writeFileSync(popupPath, definitiveWorkingPopup, 'utf8');
console.log('popup.js updated with multi-row automatic creation!');
