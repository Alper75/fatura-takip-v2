...
const definitivePopupCode = `document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 6. Mahsup Fişine Aktar (Bilanço - fisDetayFormArray ve listFisDetayAction uyumlu)
    if (mahsupPasteBtn) {
        mahsupPasteBtn.addEventListener('click', async () => {
            ...
            try {
                await chrome.scripting.executeScript({
                    target: { tabId: tab.id, allFrames: true },
                    func: async (rows) => {
                        // Sadece Fiş Detayı Formunun veya Tablosunun bulunduğu frame'de çalış
                        ...
                        const getFields = () => {
                            const kods = Array.from(document.querySelectorAll('input[name="HESAP_KODU"], input[name*="hesapKodu"]'));
                            const evrakNos = Array.from(document.querySelectorAll('input[name="EVRAK_NO"], input[name*="evrakNo"]'));
                            const tarihs = Array.from(document.querySelectorAll('input[name="EVRAK_TARIH"], input[name*="evrakTarih"], input[name*="TARIH"]'));
                            const belgeTurs = Array.from(document.querySelectorAll('input[name="BELGE_TURU"], input[name*="belgeTur"]'));
                            const aciklamas = Array.from(document.querySelectorAll('input[name="ACIKLAMA"], input[name*="aciklama"]'));
                            const borcs = Array.from(document.querySelectorAll('input[name="BORC"], input[name*="borc"]'));
                            const alacaks = Array.from(document.querySelectorAll('input[name="ALACAK"], input[name*="alacak"]'));
                        ...
                        for (let i = 0; i < rows.length; i++) {
                            ...
                            // Eğer satır i henüz yoksa yeni satır aç
                            if (!fields.kods[i] && i > 0) {
                                addNewRow();
                                let attempts = 0;
                                while (!getFields().kods[i] && attempts < 50) {
                            ...
                            if (!fields.kods[i]) {
                                // Fallback: tr üzerinden ara
                                const allTrs = Array.from(document.querySelectorAll('tr')).filter(tr => tr.querySelectorAll('input').length >= 4);
                                if (allTrs[i]) {
                                    const inps = Array.from(allTrs[i].querySelectorAll('input[type="text"], input:not([type="hidden"]):not([type="checkbox"])'));
                                    if (inps[0]) {
                                        const kodVal = (r.muhasebeKodu || r.hesapKodu || r['Hesap Kodu'] || '').trim();
                                        setVal(inps[0], kodVal);
                                        inps[0].dispatchEvent(new KeyboardEvent('keydown', { key: 'Tab', keyCode: 9, which: 9, bubbles: true }));
                                        if (inps[1]) setVal(inps[1], r.evrakNo || r.no || r.faturaNo || '');
                                        if (inps[2]) setVal(inps[2], formatDateTR(r.tarih || r.evrakTarihi || ''));
                                        if (inps[3]) setVal(inps[3], r.belgeTuru || 'FT');
                                        if (inps[4]) setVal(inps[4], r.aciklama || r.unvan || '');
                                        const bVal = r.borc !== undefined && Number(r.borc) > 0 ? r.borc : (r.tur === 'borc' ? r.tutar : 0);
                                        const aVal = r.alacak !== undefined && Number(r.alacak) > 0 ? r.alacak : (r.tur === 'alacak' ? r.tutar : 0);
                                        if (bVal && Number(bVal) > 0 && inps[5]) setVal(inps[5], formatTutarTR(bVal));
                                        else if (aVal && Number(aVal) > 0 && inps[6]) setVal(inps[6], formatTutarTR(aVal));
                                    }
                                }
                                continue;
                            }
...
fs.writeFileSync(popupPath, definitivePopupCode, 'utf8');
console.log('popup.js updated with exact fisDetayFormArray named fields mapping!');
