...
const finalPerfectPopupCode = `document.addEventListener('DOMContentLoaded', async () => {
            ...
            try {
                await chrome.scripting.executeScript({
                    ...
                    func: async (rows) => {
                        ...
                        // SADECE GÖRÜNÜR (type!=hidden) TABLO İNPUTLARINI ALARAK ÜSTTEKİ GİZLİ TARİHLERİ ENGELLİYORUZ
                        const getFields = () => {
                            const isVisibleText = (el) => el && el.type !== 'hidden' && el.type !== 'checkbox' && el.type !== 'radio';
                            const kods = Array.from(document.querySelectorAll('input[name="HESAP_KODU"]')).filter(isVisibleText);
                            const evrakNos = Array.from(document.querySelectorAll('input[name="EVRAK_NO"]')).filter(isVisibleText);
                            const tarihs = Array.from(document.querySelectorAll('input[name="EVRAK_TARIH"], input[name="EVRAK_TARIHI"]')).filter(isVisibleText);
                            const belgeTurs = Array.from(document.querySelectorAll('input[name="BELGE_TURU"]')).filter(isVisibleText);
                            const aciklamas = Array.from(document.querySelectorAll('input[name="ACIKLAMA"]')).filter(isVisibleText);
                            const borcs = Array.from(document.querySelectorAll('input[name="BORC"]')).filter(isVisibleText);
                            const alacaks = Array.from(document.querySelectorAll('input[name="ALACAK"]')).filter(isVisibleText);
                        ...
                        for (let i = 0; i < rows.length; i++) {
                            ...
                            // Eğer i. satır inputu henüz yoksa yeni satır aç
...
fs.writeFileSync(popupPath, finalPerfectPopupCode, 'utf8');
console.log('popup.js updated with filtered visible fields mapping!');
