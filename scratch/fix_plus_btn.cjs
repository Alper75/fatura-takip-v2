...
const definitiveWorkingPopupWithAddRow = `document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 6. Mahsup Fişine Aktar (Bilanço - addRow & td.add_delete desteği)
    if (mahsupPasteBtn) {
            ...
            try {
                await chrome.scripting.executeScript({
                    ...
                    func: async (rows) => {
                        ...
                        // KESİN SATIR EKLEME: td.add_delete input[value="+"] + onclick="degistimi=true;addRow(this, 3);"
                        const triggerAddRow = async () => {
                            // 1. Son satırdaki "+" butonuna tıkla
                            const plusBtns = Array.from(document.querySelectorAll('td.add_delete input[value="+"], input[title*="Ekle"], input[value="+"], input[onclick*="addRow"]'));
                            if (plusBtns.length > 0) {
                                const lastBtn = plusBtns[plusBtns.length - 1];
                                try {
                                    lastBtn.focus();
                                    lastBtn.click();
                                    return;
                            ...
                            // 2. addRow fonksiyonunu çağır
                            if (typeof window.addRow === 'function') {
                                try { window.addRow(null, 3); return; } catch(e) {}
                            }
                            if (typeof window.satirEkle === 'function') {
                                try { window.satirEkle(); return; } catch(e) {}
                            }
                        ...
                        for (let i = 0; i < rows.length; i++) {
                            ...
                            // Eğer satır i henüz yoksa "+" butonuna basıp açılmasını bekle
                            if (!fields.kods[i] && i > 0) {
                                await triggerAddRow();
                                let attempts = 0;
                                while (!getFields().kods[i] && attempts < 25) {
                            ...
                            // Sıradaki satırı açmak için doğrudan satırdaki "+" butonuna tıkla
                            if (i < rows.length - 1) {
                                await triggerAddRow();
                                await new Promise(res => setTimeout(res, 40));
                            }
...
fs.writeFileSync(popupPath, definitiveWorkingPopupWithAddRow, 'utf8');
console.log('popup.js updated with exact td.add_delete plus button click and addRow(this, 3)!');
