...
const perfectPopupCode = `document.addEventListener('DOMContentLoaded', async () => {
        ...
        mahsupPasteBtn.addEventListener('click', async () => {
            ...
            try {
                await chrome.scripting.executeScript({
                    target: { tabId: tab.id, allFrames: true },
                    func: async (rows) => {
                        // Sadece Fiş Tablosunun (#tr1 veya #scroll) bulunduğu frame'de çalış
                        const hasFisTable = !!document.getElementById('tr1') || !!document.querySelector('#scroll');
                        if (!hasFisTable) return false;
                        ...
                        for (let i = 0; i < rows.length; i++) {
                            const r = rows[i];
                            const rowIdx = i + 1;
                            let tr = document.getElementById('tr' + rowIdx);

                            if (!tr && i > 0) {
                                pressAltE();
                                let attempts = 0;
                                while (!document.getElementById('tr' + rowIdx) && attempts < 50) {
                                    await new Promise(res => setTimeout(res, 80));
                                    attempts++;
                                }
                                tr = document.getElementById('tr' + rowIdx);
                            }

                            if (!tr) break;

                            // tr içindeki görünür tüm text inputlarını al
                            const inputs = Array.from(tr.querySelectorAll('input[type="text"], input:not([type="hidden"]):not([type="checkbox"]):not([type="radio"])'));

                            // 1. Hesap Kodu (inputs[0])
                            const kodVal = (r.muhasebeKodu || r.hesapKodu || r['Hesap Kodu'] || '').trim();
                            if (inputs[0] && kodVal) {
                                setVal(inputs[0], kodVal);
                            ...
                            const evrakNoInput = tr.querySelector('[name*="EVRAK_NO"]') || inputs[1] || (tr.children[3] ? tr.children[3].querySelector('input') : null);
                            ...
                            const evrakTarihInput = tr.querySelector('[name*="EVRAK_TARIH"], [name*="TARIH"]') || inputs[2] || (tr.children[4] ? tr.children[4].querySelector('input') : null);
                            if (evrakTarihInput && tarihVal) setVal(evrakTarihInput, formatDateTR(tarihVal));

                            // 4. Belge Türü
                            const belgeTurInput = tr.querySelector('[name*="BELGE_TUR"]') || inputs[3] || (tr.children[5] ? tr.children[5].querySelector('input') : null);
                            if (belgeTurInput) setVal(belgeTurInput, r.belgeTuru || r['Belge Türü'] || 'FT');

                            // 5. Açıklama
                            const aciklamaVal = r.aciklama || r.unvan || r['Detay Açıklama'] || '';
                            const aciklamaInput = tr.querySelector('[name*="ACIKLAMA"]') || inputs[4] || (tr.children[7] ? tr.children[7].querySelector('input') : null);
                            ...
                            const borcInput = tr.querySelector('[name*="BORC"]') || inputs[5] || (tr.children[8] ? tr.children[8].querySelector('input') : null);
                            const alacakInput = tr.querySelector('[name*="ALACAK"]') || inputs[6] || (tr.children[9] ? tr.children[9].querySelector('input') : null);
...
fs.writeFileSync(popupPath, perfectPopupCode, 'utf8');
console.log('popup.js completely rewritten and syntax validated!');
